...
The data returned from SquashInfo seem to imply that this is not the case! Age, height, weight, and BMI do not seem to have any clear relationship with PSA rankings. There are many factors that contribute to player success, including endurance, positioning, accuracy, and play-style.  
&nbsp;  
...
```
&nbsp;  
`get_rankings()` is useful for analyzing how player rankings have changed recently. Alternatively, `get_historical_rankings()` will pull a rankings table from any given month and year. Although, this function takes time to run, because it constructs historical rankings from individual player ranking histories. These individual histories can be accessed through `get_player_ranking_history()`.  
&nbsp;  
...
The averages for Karim Abdel Gawad, Simon Rösner, and Paul Coll are particularly high. This reflects how they defeated many of their opponents up to the third round of the tournament.  
